...
//// 选择陀螺仪
#define IST8310 // 如果使用A板自带的陀螺仪就使能为1
...
#define YAW_SPEED_PID_KD 2000.0f
...
#define YAW_GYRO_ABSOLUTE_PID_KP 25.0f //原10
#define YAW_GYRO_ABSOLUTE_PID_KI 0.0f		
#define YAW_GYRO_ABSOLUTE_PID_KD 0.4f  //0.3
#define YAW_GYRO_ABSOLUTE_PID_MAX_OUT 10.0f   //原10
...
#define INIT_YAW_SET 0.0f     //0代表头朝前
#define INIT_PITCH_SET 0.0f		//0代表俯仰水平
...
#define GIMBAL_YAW_RELATIVE_ANGLE_OFFSET 7474		//按步兵V2的装配，实际装配偏差
#define GIMBAL_PITCH_RELATIVE_ANGLE_OFFSET 7200    //还没有调Pitch轴，所以值为0
...
#ifdef WT931 //维特智能IMU 额外买的 接UART8
typedef struct
{
    const RC_ctrl_t *gimbal_rc_ctrl;    //遥控器指针
		const short *gimbal_INT_angle_point;	//short用于维特智能 WT931 IMU
		const short *gimbal_INT_gyro_point;
    Gimbal_Motor_t gimbal_yaw_motor;    //YAW电机结构体
    Gimbal_Motor_t gimbal_pitch_motor;	//PITCH电机结构体
    Gimbal_Cali_t gimbal_cali;					//云台校准结构体
} Gimbal_Control_t;
#endif

#ifdef IST8310 //A板自带的IMU 用的SPI
typedef struct
{
    const RC_ctrl_t *gimbal_rc_ctrl;    //遥控器指针
    const fp32 *gimbal_INT_angle_point; //IMU角度指针 						// fp32用于A板自带IMU
    const fp32 *gimbal_INT_gyro_point;	//IMU的陀螺仪角速度指针		// fp32用于A板自带IMU
    Gimbal_Motor_t gimbal_yaw_motor;    //YAW电机结构体
    Gimbal_Motor_t gimbal_pitch_motor;	//PITCH电机结构体
    Gimbal_Cali_t gimbal_cali;					//云台校准结构体
} Gimbal_Control_t;
