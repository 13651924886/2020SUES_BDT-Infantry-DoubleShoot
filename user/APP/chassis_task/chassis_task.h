...
typedef struct
{
	const extended_mcu_msg_t *super_power_mcu_msg_pointer;
	
	Power_State_e charge_cmd_state; 			//降压模块期望要切换的模式（给超级电容充电）
	Power_State_e discharge_cmd_state;		//升压模块期望要切换的模式（超级电容放电给底盘）
	Power_State_e	charge_current_state;		//真实模式
	Power_State_e	discharge_current_state;//真实模式
	
	float voltage; //超级电容电压
...
typedef struct
{
  ...
  fp32 chassis_roll;  //陀螺仪和云台电机叠加的roll角度
	...
	SuperCap_t super_cap;

} chassis_move_t;

extern const float *get_Voltage_Point(void);
